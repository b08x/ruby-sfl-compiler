...
- KB formatters (`kb_csv/json/markdown_formatter.rb` + `kb_report_writer.rb`) mirror the same trio pattern for `Types::KnowledgeBaseReport` (`kb_migration.csv/json/md`, artifact-level aggregates only — quality score, content type, migration action). `kb_annotated_doc_formatter.rb` + `kb_annotated_doc_writer.rb` are the exception: opt-in via `--annotated`, operate per-`KnowledgeArtifact` (not per-report), and write one file per *source document* to `annotated/` under `--output-dir` — the formatter renders one artifact's clauses as a `##` section (tagged inline `[process_type · mood · tenor=N · modality=N]`, order preserved from `KnowledgeArtifact#clauses`), the writer groups artifacts sharing a `source_file` into one document (a file with multiple headings, or a text + image pass, produces >1 artifact) and disambiguates same-basename files from different directories.
...
- **Scope today**: `topics: nil` path only (no topic-modeling pre-pass for documentation). Both reduce jobs now include `turns:` in `full_output` so `Types.load_analysis_result` can reconstruct the full result for report writing. `--live` wires through the TUI via `WorkflowPoller` + `BatchApp` for both `conversation` and `documentation` subcommands; one file per invocation (multi-file deferred).
...
11. **`bundle exec` and exe/**: the Gemfile *does* use `gemspec` (verified — `sfl-compiler` shows up as a `PATH` source in `Gemfile.lock`), so `bundle exec ruby -e 'require "sfl-compiler"'` puts `lib/` on the load path with no extra wiring. `exe/sfl-analyze`'s own `$LOAD_PATH.unshift` guard is redundant under `bundle exec` but still needed for running it directly (`ruby exe/sfl-analyze` or an installed binstub outside Bundler). Run specs with `bin/rspec` if the `bundle exec rspec` binstub misbehaves.
...
14. **`vendor/python` only gets populated by a real `gem install`, not `bundle install`**: `ext/sfl_compiler/extconf.rb` vendors spaCy + `en_core_web_sm` via `uv` (bootstrapping `uv` itself into `vendor/uv/` if not on `PATH`) and pins `numpy<2` explicitly — an unpinned `spacy` install resolves NumPy 2.x, which breaks the Ruby `numpy` gem (`0.4.0`, the only release, a `pycall` dependency) by removing `numpy.chararray`. Bundler treats `gemspec`/`path`-sourced gems as already "installed" in place and never runs their extension build step, so this repo's own `bundle install` does **not** trigger vendoring — run `ruby ext/sfl_compiler/extconf.rb` by hand for a working local `vendor/` dir. At runtime, `Bootstrap.configure_vendored_python` (called first thing inside `Bootstrap.call`) reads `vendor/python/.sfl_compiler_python_path` and sets `ENV['PYTHON']`/`PYTHONPATH` before anything `require`s `ruby-spacy` — an explicit `ENV['PYTHON']` always wins. No `vendor/` present → silent no-op, falls back to whatever `python3` resolves to on the host.
