...
talk to existential philosophy. CBT does not talk to Unix. The ruby-sfl-compiler
...
the compiler traces back to a specific intellectual tradition. The synthesis is
...
The compiler reverse-engineers Halliday's metafunctions into programmatic,
...
existential — using lemma-based verb lists. The `PassTwoEngine` (Pass 2,
LLM-based) annotates each clause's interpersonal features: mood, modality
weight, tenor, speaker attitude.

The key engineering move is **payload separation** (see
[README — Payload Separation](../README.md#payload-separation)). SFL
...
The compiler borrows the practice of identifying cognitive distortions and
applies it programmatically via **stance filtering**. During RAG retrieval,
the `HybridRetriever` applies scalar filters on the interpersonal payload:
...
The compiler's Phase 2 **Rolling Synthesis** pattern (see
[ROADMAP.md](../ROADMAP.md)) mimics neural memory consolidation. Instead of
forcing an LLM to hold infinite tokens in its context window — which degrades
reasoning quality as the window grows — the system periodically compresses
short-term raw text into dense, long-term "Axiomatic" semantic summaries.

The mechanism: intermediate Genie syntheses fire at semantic boundaries. Each
synthesis compresses the working memory of clauses processed so far into a
compact Axiomatic summary that preserves SFL metadata (process types,
modality weights, tenor scores) but discards raw token sequences. The raw
clauses remain in PostgreSQL; the context window carries only the summary
forward.
...
reports, multi-day conversation trees — without the context window
degrading into noise. The reasoning quality at cycle N+100 matches the
reasoning quality at cycle 1, because each cycle starts from a compressed
semantic summary, not from an ever-growing pile of raw tokens.
...
The compiler physically separates the **ideational payload** (facticity —
...
See [docs/use-cases/llm-role-isolation.md — De-fanging the
...
(spaCy via PyCall) does one thing: syntactic parsing. It produces
`SyntacticClause` and `IdeationalPayload` objects — clean, standardized
data structures that pass down the pipe. Pass 2 (DSPy.rb + LLM) does one
thing: semantic annotation. It consumes Pass 1's output and produces
`AnnotatedClause` objects. The two passes do not share state, do not
call each other, and can be run independently.

The pipeline enforces this separation with `Dry::Struct` type contracts
(see `lib/sfl/compiler/types.rb`) — the "velvet rope" that validates every
...
This extends to the analysis layer: `ConversationAnalyzer`,
`DocumentationAnalyzer`, `TenorTracker`, `SpeakerProfiler`, and
`CorrelationAnalyzer` are independent modules that consume the pipeline's
output and produce their own. None of them touch `ENV`, none of them print,
none of them call each other directly. They are composable Unix tools with
injected dependencies, designed to back a CLI today and a TUI or web UI
...
the system extensible — and what makes Phase 2 (decoupling the runtimes)
...
The compiler adapts air gapping and data sanitization to **semantic context**.
The `HybridRetriever`'s scalar stance filters act as a **Rhetorical Firewall**
— a gate that operates before the LLM's context window, not after.

During retrieval, the RRF merge produces a ranked list of candidate clauses.
The `apply_filters` method (see `lib/sfl/compiler/retrieval/hybrid_retriever.rb`,
lines 164–199) then evaluates each candidate against the interpersonal
...
layer below: it removes the manipulative content from the channel before
the model has a chance to be influenced by it. The defense is structural,
not behavioral.

See [docs/use-cases/llm-role-isolation.md — The Single-Channel
Illusion](use-cases/llm-role-isolation.md#the-single-channel-illusion)
...
provides the distortion taxonomy but not the retrieval mechanism.
Cognitive neuroscience provides the consolidation pattern but not the
linguistic annotation. Existential philosophy provides the facticity/
interpretation distinction but not the storage schema. The Unix philosophy
provides the composability but not the security posture. Cybersecurity
provides the air-gapping model but not the semantic filter dimensions.

The ruby-sfl-compiler is the point where these threads converge:
...
- Neural consolidation becomes the context management pattern (Phase 2
  Rolling Synthesis implements it).
...
The compiler's two-pass annotation pipeline, payload separation schema, and
...
Phase 1 proved the pipeline works. The current work is proving the
filters change outcomes. Phase 2 will prove it scales. The end state is a
framework that any RAG system can adopt to gain a structural defense
against context poisoning — without rewriting its existing architecture.
...
- [docs/architecture.md](architecture.md) — QuestionGraph, Gödel numbering, pipeline architecture
- [docs/decisions.md](decisions.md) — design decisions and rationale
- [lib/sfl/compiler/types.rb](../lib/sfl/compiler/types.rb) — IdeationalPayload, InterpersonalPayload
- [lib/sfl/compiler/retrieval/hybrid_retriever.rb](../lib/sfl/compiler/retrieval/hybrid_retriever.rb) — the `apply_filters` method